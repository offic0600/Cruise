...
| CAP-06 | 枚举 Active issues 顶部工具栏与 tabs 的交互证据 | blocked | CAP-03, CAP-04, CAP-05 | 为当前 Active issues 页的顶部 tabs / 搜索 / filter / display / sort / new 等控件逐项补前后状态、截图/DOM/网络摘要，并新增对应 flow 文档 | 2026-04-18 23:52 CST 最小复测：terminal 直连 `http://127.0.0.1:9222/json/version` 与 `/json/list` 再次都返回 `502 Bad Gateway`，但 Hermes browser 仍可读取同一组 metadata，并继续读到 Linear target=`Cleantrack › Active issues`（id=`81A3713C33BCA35B2A0B8C7D177F43AD`）；同时 `/json/list` 还能枚举到 `Codex Proxy Developer Dashboard`、GitHub commit 页面、Kimi 聊天页等其它实时目标，说明 metadata 目录仍为 browser 侧实时可读。真正打开 `https://linear.app/cleantrack/team/CLE/active` 时仍先落到 `Link opened in the Linear app` 中转页，点击 `Open here instead` 后进入 `Log in to Linear` 登录页，仅见 Google/email/SAML SSO/passkey 登录入口，仍未进入已登录 Active issues 真页；CAP-06 当前 blocker 应继续归类为 metadata split state + authenticated page/session reuse failure。最新 blocker 证据见 `docs/linear-parity/har/2026-04-18-235241-browser-terminal-metadata-discrepancy.json`、`docs/linear-parity/flows/active-issues-auth-session-blocker-2026-04-18-235241.md` |
