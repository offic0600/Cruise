...
| IMP-35 | 评估 legacy `/issues/[id]` route API 失败场景测试标题是否还可继续统一为 shared empty props contract 语义 | pending | IMP-34 | 在不扩大实现面的前提下，复核 legacy `/issues/[id]` route 的 `getIssue(...)` / `getOrganizations()` 失败场景测试标题是否仍混用 empty back-link 与 empty props contract 语义；若可行，则只做一刀测试文案收口并保持定向验证通过。 | - |
