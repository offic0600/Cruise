...
| IMP-23 | 评估并收紧 IssueDetailPage 内部 workspace/team 默认 back-link fallback 的保留边界 | pending | IMP-22 | 在不扩大到 capture lane 或整页导航重构的前提下，核对 `IssueDetailPage` 内部 `teamActivePath(...)` fallback 是否仍为唯一必要兜底；若可进一步收紧则完成最小实现与验证，否则把必须保留 fallback 的原因写回状态与 task-board | 暂无新增外部 blocker；下一轮应先确认除 route 外是否还有直接渲染 `IssueDetailPage` 且依赖内部 fallback 的调用点，再决定能否安全移除或收紧该 fallback |
