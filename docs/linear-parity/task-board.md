...
| IMP-20 | 继续收口 issue detail route 与 IssueDetailPage 之间剩余 skeleton/empty-state seam | pending | IMP-19 | 在不触碰 capture lane 的前提下，继续寻找 issue detail route 与 `IssueDetailPage` 之间仍重复的 skeleton/empty-state 或 back-link seam，并完成一个最小可验证收口增量的实现、验证、提交、状态写回与推送闭环 | 暂无新增外部 blocker；下一轮应先基于当前 route shell 已消费的前提，选择一处仍然重复的 page/route seam 做单点收口，避免扩大到整页重构 |
