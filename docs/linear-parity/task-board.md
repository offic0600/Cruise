...
| IMP-30 | 评估 legacy `/issues/[id]` route 无 slug 空 back-link 断言是否可与失败 helper 进一步收口 | pending | IMP-29 | 在不扩大实现面的前提下，复核 legacy `/issues/[id]` route 当前“无 workspace slug”空 back-link route/helper 断言是否仍存在可安全合并的重复；若可行，则继续提炼共享测试期望并保持定向验证通过。 | - |
