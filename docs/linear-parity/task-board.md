...
| IMP-33 | 评估 legacy `/issues/[id]` route 空 back-link 测试文案是否还可继续统一为 shared legacy lookup seam 语义 | pending | IMP-32 | 在不扩大实现面的前提下，复核 legacy `/issues/[id]` route 相关空 back-link 测试标题/描述是否仍混用 route、shared legacy lookup seam 与 workspace slug 语义；若可行，则只做一刀测试文案收口并保持定向验证通过。 | - |
