...
| IMP-36 | 评估 legacy `/issues/[id]` route workspace slug miss 场景测试标题是否还可继续统一为 shared empty props contract 语义 | pending | IMP-35 | 在不扩大实现面的前提下，复核 legacy `/issues/[id]` route 当前 workspace slug miss 场景 route 级测试标题是否仍保留 empty back-link wording；若可行，则只做一刀测试文案收口，使其与 shared empty props contract 语义一致并保持定向验证通过。 | - |
