...
| IMP-25 | 为 `/issues/[id]` 直达页补最小 route seam 回归测试以锁定显式 back-link contract | pending | IMP-24 | 为 `/issues/[id]` 直达 issue 详情页新增最小 route seam 回归测试，锁定其向 `IssueDetailPage` 显式透传 `href/label` back-link contract，并覆盖无可解析 workspace slug 时的安全降级行为 | 暂无外部 blocker；当前代码路径已显式传入 back-link contract，但仍缺少针对 legacy `/issues/[id]` 直达页的独立 seam 测试，下一轮应优先补齐该验证而不是继续扩大导航重构范围 |
