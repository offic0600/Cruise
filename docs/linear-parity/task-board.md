...
| IMP-27 | 评估 legacy `/issues/[id]` route 是否可抽出共享 issue/org lookup seam 以消除重复 Promise 组装 | pending | IMP-26 | 识别 legacy `/issues/[id]` route 与相关 issue detail routes 是否仍存在可最小收口的 issue/org lookup 重复组装；若存在，则落一个只触及单一 seam 的 helper 或 contract 提炼，并补最小回归测试。 | - |
