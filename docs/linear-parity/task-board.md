...
| IMP-21 | 收口 issue detail route 对 IssueDetailPage 默认 back-link props 的重复组装 | pending | IMP-20 | 在不触碰 capture lane 且不扩大到整页重构的前提下，让 issue detail route 对 `IssueDetailPage` 的默认 back-link props 也统一消费共享 shell/page-back-link contract，并完成最小验证、提交、状态写回与推送闭环 | 暂无新增外部 blocker；下一轮应在现有 `pageBackLink` seam 基础上继续处理默认 page 分支的重复 back-link 组装，避免扩大到 `IssueDetailPage` 内部 skeleton 重构 |
