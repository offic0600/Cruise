...
| IMP-31 | 评估 legacy `/issues/[id]` route 空 back-link helper 是否可继续收口 route/helper 命名与调用边界 | pending | IMP-30 | 在不扩大实现面的前提下，复核 legacy `/issues/[id]` route 当前 `expectLegacyIssueRouteEmptyBackLinkContract()` 与 helper 级无 slug 空对象断言的命名/调用边界是否仍存在可安全收紧的重复；若可行，则继续只做一刀测试收口并保持定向验证通过。 | - |
