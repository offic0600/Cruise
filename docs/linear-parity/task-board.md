...
| IMP-28 | 为 legacy `/issues/[id]` lookup seam 补最小 route 级失败/成功 contract 断言去除 route 与 helper 断言重复 | pending | IMP-27 | 在不扩大实现面的前提下，复核 `loadIssueDetailRouteLookup(...)` / `buildIssueDetailRouteBackLinkProps(...)` 与 legacy route 现有测试边界，若存在 route 级与 helper 级重复断言，则收紧为一组更稳定的最小 contract 断言并保持定向验证通过。 | - |
