...
| IMP-34 | 评估 legacy `/issues/[id]` route 空 back-link helper/contract 测试命名是否还可继续去掉重复 empty back-link 短语 | pending | IMP-33 | 在不扩大实现面的前提下，复核 `expectLegacyIssueDetailRouteEmptyBackLinkProps()` 与 `expectLegacyIssueRouteEmptyBackLinkContract()` 及其调用测试标题是否仍存在可安全压缩的重复 empty/back-link 词组；若可行，则只做一刀测试命名收口并保持定向验证通过。 | - |
