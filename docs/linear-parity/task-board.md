...
| IMP-19 | 让 issue detail route 真正消费共享 shell model 渲染 loading/empty-state 分支 | pending | IMP-18 | 把 issue detail route 的 loading/not-found/error/no-id 分支改为消费单一共享 shell model，而不是分别手写 skeleton/empty-state 组装；完成最小充分验证、review、提交、状态写回与推送闭环 | 暂无新增外部 blocker；下一轮只需限制在 route 分支消费 seam，不扩大到 `IssueDetailPage` 内部重构 |
