...
| IMP-37 | 评估 legacy `/issues/[id]` route helper 级 workspace slug miss 场景测试标题是否还可继续统一为 shared empty props contract 语义 | pending | IMP-36 | 在不扩大实现面的前提下，复核 legacy `/issues/[id]` route helper 级 workspace slug miss 场景测试标题 `falls back to an empty props contract when the shared legacy lookup seam cannot resolve a workspace slug` 是否仍可安全收口为 shared empty props contract 语义；若可行，则只做一刀测试文案收口并保持定向验证通过。 | - |
