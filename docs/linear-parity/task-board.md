...
| IMP-32 | 评估 legacy `/issues/[id]` route 空 back-link helper 命名是否可进一步对齐 legacy/route 语义 | pending | IMP-31 | 在不扩大实现面的前提下，复核 `expectEmptyLegacyIssueDetailRouteBackLinkProps()` 与 `expectLegacyIssueRouteEmptyBackLinkContract()` 的命名是否仍存在 legacy/route/helper 语义不够一致的轻微噪音；若可行，则只做一刀测试命名收口并保持定向验证通过。 | - |
