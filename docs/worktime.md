...
| S315 | 2026-04-21 | 重新评估并恢复第二条 legacy route API 失败标题中的 route wording | 0.5h | 0.5h | IMP-200：将 `getOrganizations rejects` 恢复为 `route getOrganizations rejects`，定向测试/tsc/diff 均通过 |
| S314 | 2026-04-21 | 重新评估并恢复第一条 legacy route API 失败标题中的 route wording | 0.5h | 0.5h | IMP-199：将 `getIssue rejects` 恢复为 `route getIssue rejects`，定向测试/tsc/diff 均通过 |
| S313 | 2026-04-21 | 重新评估并再次收口第二条 legacy route API 失败标题中的 route wording | 0.5h | 0.5h | IMP-198：将 `route getOrganizations rejects` 再次收口为 `getOrganizations rejects`，定向测试/tsc/diff 均通过 |
| S311 | 2026-04-21 | 重新评估并恢复第二条 legacy route API 失败标题中的 route wording | 0.5h | 0.5h | IMP-196：将 `getOrganizations rejects` 恢复为 `route getOrganizations rejects`，定向测试/tsc/diff 均通过 |
| S310 | 2026-04-21 | 重新评估并恢复第一条 legacy route API 失败标题中的 route wording | 0.5h | 0.5h | IMP-195：将 `getIssue rejects` 恢复为 `route getIssue rejects`，定向测试/tsc/diff 均通过 |
| S309 | 2026-04-21 | 重新评估并再次收口第二条 legacy route API 失败标题中的 route wording | 0.5h | 0.5h | IMP-194：将 `route getOrganizations rejects` 再次收口为 `getOrganizations rejects`，定向测试/tsc/diff 均通过 |
| S308 | 2026-04-21 | 重新评估并再次收口第一条 legacy route API 失败标题中的 route wording | 0.5h | 0.5h | IMP-193：将 `route getIssue rejects` 再次收口为 `getIssue rejects`，定向测试/tsc/diff 均通过 |
| S306 | 2026-04-21 | 重新评估并恢复第一条 legacy route API 失败标题中的 route wording | 0.5h | 0.5h | IMP-191：将 `getIssue rejects` 恢复为 `route getIssue rejects`，定向测试/tsc/diff 均通过 |
| S305 | 2026-04-21 | 重新评估并再次收口第二条 legacy route API 失败标题中的 route wording | 0.5h | 0.5h | IMP-190：将 `route getOrganizations rejects` 再次收口为 `getOrganizations rejects`，定向测试/tsc/diff 均通过 |
| S304 | 2026-04-21 | 重新评估并再次收口第一条 legacy route API 失败标题中的 route wording | 0.5h | 0.5h | IMP-189：将 `route getIssue rejects` 再次收口为 `getIssue rejects`，定向测试/tsc/diff 均通过 |
| S303 | 2026-04-21 | 重新评估并恢复第二条 legacy route API 失败标题中的 route wording | 0.5h | 0.5h | IMP-188：将 `getOrganizations rejects` 恢复为 `route getOrganizations rejects`，定向测试/tsc/diff 均通过 |
| S302 | 2026-04-21 | 重新评估并恢复第一条 legacy route API 失败标题中的 route wording | 0.5h | 0.5h | IMP-187：将 `getIssue rejects` 恢复为 `route getIssue rejects`，定向测试/tsc/diff 均通过 |
...
| 2026-04-21 18:11 | 0.2h | Session 300 / Linear parity implementation cron：完成 IMP-209，复核 legacy `/issues/[id]` route 在 `route getIssue rejects` / `route getOrganizations rejects` 组合下第一条仍可再次收口为无 route wording，仅将测试标题改为 `getIssue rejects`；验证 `pnpm test -- --run src/lib/routes.test.tsx`、`./node_modules/.bin/tsc --noEmit`、`git diff --check` 通过，并同步更新 roadmap-state / task-board / dev-logbook |
...
| 2026-04-21 10:22 | 0.3h | Session 286 / Linear parity implementation cron：完成 IMP-169，复核 legacy `/issues/[id]` route 在 `route getIssue rejects` / `route getOrganizations rejects` 组合下第一条仍可再次收口为无 route wording，仅将测试标题改为 `getIssue rejects`；验证 `pnpm test -- --run src/lib/routes.test.tsx`、`./node_modules/.bin/tsc --noEmit`、`git diff --check` 通过，并同步更新 roadmap-state / task-board / dev-logbook |
| 2026-04-21 10:39 | 0.3h | Session 287 / Linear parity implementation cron：完成 IMP-170，复核 legacy `/issues/[id]` route 在 `getIssue rejects` / `route getOrganizations rejects` 组合下第二条仍可再次收口为无 route wording，仅将测试标题改为 `getOrganizations rejects`；验证 `pnpm test -- --run src/lib/routes.test.tsx`、`./node_modules/.bin/tsc --noEmit`、`git diff --check` 通过，并同步更新 roadmap-state / task-board / dev-logbook |
| 2026-04-21 10:46 | 0.3h | Session 288 / Linear parity implementation cron：完成 IMP-171，复核 legacy `/issues/[id]` route 在 `getIssue rejects` / `getOrganizations rejects` 组合下第一条需恢复 route 级语义词，仅将测试标题改为 `route getIssue rejects`；验证 `pnpm test -- --run src/lib/routes.test.tsx`、`./node_modules/.bin/tsc --noEmit`、`git diff --check` 通过，并同步更新 roadmap-state / task-board / dev-logbook |
| 2026-04-21 12:36 | 0.3h | Session 293 / Linear parity implementation cron：完成 IMP-178，复核 legacy `/issues/[id]` route 在 `getIssue rejects` / `route getOrganizations rejects` 组合下第二条仍可再次收口为无 route wording，仅将测试标题改为 `getOrganizations rejects`；验证 `pnpm test -- --run src/lib/routes.test.tsx`、`./node_modules/.bin/tsc --noEmit`、`git diff --check` 通过，并同步更新 roadmap-state / task-board / dev-logbook |
| 2026-04-21 12:53 | 0.2h | Session 294 / Linear parity implementation cron：完成 IMP-179，复核 legacy `/issues/[id]` route 在 `getIssue rejects` / `getOrganizations rejects` 组合下第一条需恢复 route 级语义词，仅将测试标题改为 `route getIssue rejects`；验证 `pnpm test -- --run src/lib/routes.test.tsx`、`./node_modules/.bin/tsc --noEmit`、`git diff --check` 通过，并同步更新 roadmap-state / task-board / dev-logbook |
| 2026-04-21 13:25 | 0.2h | Session 296 / Linear parity implementation cron：完成 IMP-181，复核 legacy `/issues/[id]` route 在 `route getIssue rejects` / `route getOrganizations rejects` 组合下第一条仍可再次收口为无 route wording，仅将测试标题改为 `getIssue rejects`；验证 `pnpm test -- --run src/lib/routes.test.tsx`、`./node_modules/.bin/tsc --noEmit`、`git diff --check` 通过，并同步更新 roadmap-state / task-board / dev-logbook |
| 2026-04-21 13:40 | 0.2h | Session 297 / Linear parity implementation cron：完成 IMP-182，复核 legacy `/issues/[id]` route 在 `getIssue rejects` / `route getOrganizations rejects` 组合下第二条仍可再次收口为无 route wording，仅将测试标题改为 `getOrganizations rejects`；验证 `pnpm test -- --run src/lib/routes.test.tsx`、`./node_modules/.bin/tsc --noEmit`、`git diff --check` 通过，并同步更新 roadmap-state / task-board / dev-logbook |
| 2026-04-21 14:08 | 0.2h | Session 299 / Linear parity implementation cron：完成 IMP-184，复核 legacy `/issues/[id]` route 在 `route getIssue rejects` / `getOrganizations rejects` 组合下第二条需恢复 route 级语义词，仅将测试标题改为 `route getOrganizations rejects`；验证 `pnpm test -- --run src/lib/routes.test.tsx`、`./node_modules/.bin/tsc --noEmit`、`git diff --check` 通过，并同步更新 roadmap-state / task-board / dev-logbook |
| 2026-04-21 14:17 | 0.2h | Session 300 / Linear parity implementation cron：完成 IMP-185，复核 legacy `/issues/[id]` route 在 `route getIssue rejects` / `route getOrganizations rejects` 组合下第一条仍可再次收口为无 route wording，仅将测试标题改为 `getIssue rejects`；验证 `pnpm test -- --run src/lib/routes.test.tsx`、`./node_modules/.bin/tsc --noEmit`、`git diff --check` 通过，并同步更新 roadmap-state / task-board / dev-logbook |
