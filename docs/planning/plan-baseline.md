...
### 目标
1. ✅ 技术选型确定
2. ⏳ 项目骨架搭建（Gradle monorepo + Next.js 前端）
3. ⏳ 核心数据模型设计（ERD + 实体定义）
4. ⏳ 数据库连接验证（本地 PostgreSQL 可用）
5. ⏳ 基础 API 框架（Spring Boot 启动，/health 可访问）
6. ⏳ 前端框架（Next.js 启动，首页可访问）
...
### 核心模块
- **认证**：用户注册/登录（JWT 或 Session）
- **需求管理**：CRUD + 状态流转（新建→分析→开发→测试→完成）
- **任务管理**：CRUD + 与需求关联 + 分配给人员
- **人员管理**：CRUD + 团队结构
- **开发看板**：多角色视图（至少 PM 视图 + 开发视图）
