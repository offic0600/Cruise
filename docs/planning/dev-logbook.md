...
| 步骤 | 操作 | 结果 |
|------|------|------|
| 恢复状态 | `git status --short` / `git branch --show-current` / `git rev-parse HEAD` / `git log --oneline -5` / 读取 `docs/status/roadmap-state.yaml` / `docs/linear-parity/task-board.md` / roadmap / `docs/planning/dev-logbook.md` / `docs/worktime.md` | 确认仓库位于 `codex/unify-issue-model`；起始 HEAD 为 `3859241dbac09edaa146f748116c8d0427cf42ca`，状态文件与任务板指向 `IMP-213`，工作树干净，可按唯一 execution unit 推进。 |
...
| Review | `git diff -- frontend/src/lib/routes.test.tsx` | 复核本轮只改一处 legacy route 级 API 失败测试标题文案，不改共享断言结构、fixture 设置或 helper/route 运行时逻辑；独立 reviewer 判定无安全或逻辑问题，review 结论为通过。 |
...
| 修改 | `docs/status/roadmap-state.yaml` / `docs/linear-parity/task-board.md` / `docs/planning/dev-logbook.md` / `docs/worktime.md` | 将 `IMP-213` 作为最新完成的 Implementation lane 最小增量写回，并补一条新的后继 `pending` Implementation 任务 `IMP-214` 供下一轮恢复。 |

### 326.2 本轮落地结果

- 已复核 helper 级标题仍通过 `returns the empty props contract when the helper workspace slug lookup misses` 明确保留 helper seam 语义。
- 在当前 `route getIssue rejects` / `route getOrganizations rejects` 双 route wording 组合下，第一条 API 失败标题仍可再次收口为 `getIssue rejects`，同时继续维持 route 级 API 失败场景与 helper 级 lookup 标题之间的层级区分。
...
- `docs/status/roadmap-state.yaml`、`docs/linear-parity/task-board.md`、`docs/planning/dev-logbook.md` 与 `docs/worktime.md` 已写回：`IMP-213` 作为最新完成项，下一轮转入继续评估第二条 API 失败场景标题在当前再次非对称 wording 组合下是否仍可收口为无 route wording。

### 326.3 经验沉淀

- 当两条 legacy route API 失败标题都已恢复为 route wording，而 helper 级 lookup 标题仍显式保留 helper seam 语义时，应单独复核第一条是否仍可再次收口为无 route wording，不必因为上一轮刚恢复第二条 route wording 就默认双 route wording 必须保持不变。
...
| 指标 | 值 |
|------|-----|
| 当前 lane / task | `Implementation / IMP-213 → IMP-214` |
| 当前分支 / HEAD（执行前） | `codex/unify-issue-model` / `3859241dbac09edaa146f748116c8d0427cf42ca` |
| feature commit | `b23e70d8b58064379bb72f65f50dfcb0d2f7d0b3` |
| 验证 | `pnpm test -- --run src/lib/routes.test.tsx`、`./node_modules/.bin/tsc --noEmit`、`git diff --check` |

---

## Session 325 — 2026-04-21：重新评估并恢复第二条 legacy route API 失败标题中的 route wording

**目标**：按 Implementation lane 推进 `docs/status/roadmap-state.yaml` 与 `docs/linear-parity/task-board.md` 指向的 `IMP-212` 最小增量，在检查 git/roadmap/task-board/logbook/worktime 后，只做一个最小 execution unit：基于当前 `route getIssue rejects` / `getOrganizations rejects` 组合，复核第二条 API 失败场景标题是否需要恢复为 `route getOrganizations rejects` 以继续维持与 helper 级 lookup 标题的层级区分；若需要，则仅改这一处标题并完成验证、review、提交、状态写回与 push 闭环。

### 325.1 执行步骤

| 步骤 | 操作 | 结果 |
|------|------|------|
| 恢复状态 | `git status --short` / `git branch --show-current` / `git rev-parse HEAD` / `git log --oneline -5` / 读取 `docs/status/roadmap-state.yaml` / `docs/linear-parity/task-board.md` / roadmap / `docs/planning/dev-logbook.md` / `docs/worktime.md` | 确认仓库位于 `codex/unify-issue-model`；起始 HEAD 为 `48af52b84b2d96986a30979502aaaa5ed622ae32`，状态文件与任务板指向 `IMP-212`，工作树干净，可按唯一 execution unit 推进。 |
...
| 验证 | `git diff --check` | 通过 |
| Review | `git diff -- frontend/src/lib/routes.test.tsx` | 复核本轮只改一处 legacy route 级 API 失败测试标题文案，不改共享断言结构、fixture 设置或 helper/route 运行时逻辑；独立 reviewer 判定无安全或逻辑问题，review 结论为通过。 |
| 提交 | `git commit -m "[verified] test: restore second legacy route reject title"` | 完成本轮 feature/work commit，提交后以 `git rev-parse HEAD` 记录真实 SHA。 |
| 修改 | `docs/status/roadmap-state.yaml` / `docs/linear-parity/task-board.md` / `docs/planning/dev-logbook.md` / `docs/worktime.md` | 将 `IMP-212` 作为最新完成的 Implementation lane 最小增量写回，并补一条新的后继 `pending` Implementation 任务 `IMP-213` 供下一轮恢复。 |

### 325.2 本轮落地结果

- 已复核 helper 级标题仍通过 `returns the empty props contract when the helper workspace slug lookup misses` 明确保留 helper seam 语义。
- 在当前 `route getIssue rejects` / `getOrganizations rejects` 非对称组合下，第二条 API 失败标题若继续维持无 route 级语义词，会使两条 route 级 API 失败场景与 helper 级 lookup 标题之间的层级区分再次失衡。
...
- `docs/status/roadmap-state.yaml`、`docs/linear-parity/task-board.md`、`docs/planning/dev-logbook.md` 与 `docs/worktime.md` 已写回：`IMP-212` 作为最新完成项，下一轮转入继续评估第一条 API 失败场景标题在当前再次双 route wording 组合下是否仍可收口为无 route wording。

### 325.3 经验沉淀

- 当第一条 legacy route API 失败标题已恢复为 `route getIssue rejects` 而第二条仍保持无 route wording 时，应单独复核第二条是否需要同步恢复 route 级语义词，以避免 route 级 API 失败场景与 helper 级 lookup 标题之间的层级区分再次失衡。
- 纯测试标题微调依然要在状态文件与任务板中立即补出后继 pending Implementation 任务，避免下一轮实现 cron 停在已完成项。
...
| 指标 | 值 |
|------|-----|
| 当前 lane / task | `Implementation / IMP-212 → IMP-213` |
| 当前分支 / HEAD（执行前） | `codex/unify-issue-model` / `48af52b84b2d96986a30979502aaaa5ed622ae32` |
| feature commit | `0f41949b044a50c3e55f63da8435e5819faefe0f` |
| 验证 | `pnpm test -- --run src/lib/routes.test.tsx`、`./node_modules/.bin/tsc --noEmit`、`git diff --check` |

---

## Session 324 — 2026-04-21：重新评估并恢复第一条 legacy route API 失败标题中的 route wording

**目标**：按 Implementation lane 推进 `docs/status/roadmap-state.yaml` 与 `docs/linear-parity/task-board.md` 指向的 `IMP-211` 最小增量，在检查 git/roadmap/task-board/logbook/worktime 后，只做一个最小 execution unit：基于当前 `getIssue rejects` / `getOrganizations rejects` 组合，复核第一条 API 失败场景标题是否需要恢复为 `route getIssue rejects` 以继续维持与 helper 级 lookup 标题的层级区分；若需要，则仅改这一处标题并完成验证、review、提交、状态写回与 push 闭环。

### 324.1 执行步骤

| 步骤 | 操作 | 结果 |
|------|------|------|
| 恢复状态 | `git status --short` / `git branch --show-current` / `git rev-parse HEAD` / `git log --oneline -5` / 读取 `docs/status/roadmap-state.yaml` / `docs/linear-parity/task-board.md` / roadmap / `docs/planning/dev-logbook.md` / `docs/worktime.md` | 确认仓库位于 `codex/unify-issue-model`；起始 HEAD 为 `f5bcd37d4b61e4304afcddfe2e9f147c569f7a1b`，状态文件与任务板指向 `IMP-211`，工作树干净，可按唯一 execution unit 推进。 |
...
| 验证 | `git diff --check` | 通过 |
| Review | `git diff -- frontend/src/lib/routes.test.tsx` | 复核本轮只改一处 legacy route 级 API 失败测试标题文案，不改共享断言结构、fixture 设置或 helper/route 运行时逻辑；独立 reviewer 判定无安全或逻辑问题，review 结论为通过。 |
| 提交 | `git commit -m "[verified] test: restore first legacy route reject title again"` | 完成本轮 feature/work commit，提交后以 `git rev-parse HEAD` 记录真实 SHA。 |
| 修改 | `docs/status/roadmap-state.yaml` / `docs/linear-parity/task-board.md` / `docs/planning/dev-logbook.md` / `docs/worktime.md` | 将 `IMP-211` 作为最新完成的 Implementation lane 最小增量写回，并补一条新的后继 `pending` Implementation 任务 `IMP-212` 供下一轮恢复。 |

### 324.2 本轮落地结果

- 已复核 helper 级标题仍通过 `returns the empty props contract when the helper workspace slug lookup misses` 明确保留 helper seam 语义。
- 在当前 `getIssue rejects` / `getOrganizations rejects` 双无 route wording 组合下，第一条 API 失败标题若继续维持无 route 级语义词，会与 helper 级 lookup 标题层级过于接近。
...
- `docs/status/roadmap-state.yaml`、`docs/linear-parity/task-board.md`、`docs/planning/dev-logbook.md` 与 `docs/worktime.md` 已写回：`IMP-211` 作为最新完成项，下一轮转入继续评估第二条 API 失败场景标题在当前恢复第一条 route wording 后是否需要同步恢复 route 级语义词。

### 324.3 经验沉淀

- 当两条 legacy route API 失败标题都已收口为无 route wording 时，应单独复核第一条是否需要恢复 route 级语义词，以避免与 helper 级 lookup 标题的层级过于贴近。
- 纯测试标题微调依然要在状态文件与任务板中立即补出后继 pending Implementation 任务，避免下一轮实现 cron 停在已完成项。
...
| 指标 | 值 |
|------|-----|
| 当前 lane / task | `Implementation / IMP-211 → IMP-212` |
| 当前分支 / HEAD（执行前） | `codex/unify-issue-model` / `f5bcd37d4b61e4304afcddfe2e9f147c569f7a1b` |
| feature commit | `c904d1cc99b0efeee4122a17a05684e55bd78a5d` |
| 验证 | `pnpm test -- --run src/lib/routes.test.tsx`、`./node_modules/.bin/tsc --noEmit`、`git diff --check` |

---

## Session 323 — 2026-04-21：重新评估并再次收口第二条 legacy route API 失败标题中的 route wording

**目标**：按 Implementation lane 推进 `docs/status/roadmap-state.yaml` 与 `docs/linear-parity/task-board.md` 指向的 `IMP-210` 最小增量，在检查 git/roadmap/task-board/logbook/worktime 后，只做一个最小 execution unit：基于当前 `getIssue rejects` / `route getOrganizations rejects` 组合，复核第二条 API 失败场景标题是否仍可再次收口为 `getOrganizations rejects` 以继续维持与 helper 级 lookup 标题的层级区分；若可行，则仅改这一处标题并完成验证、review、提交、状态写回与 push 闭环。

### 323.1 执行步骤

| 步骤 | 操作 | 结果 |
|------|------|------|
| 恢复状态 | `git status --short` / `git branch --show-current` / `git rev-parse HEAD` / `git log --oneline -5` / 读取 `docs/status/roadmap-state.yaml` / `docs/linear-parity/task-board.md` / roadmap / `docs/planning/dev-logbook.md` / `docs/worktime.md` | 确认仓库位于 `codex/unify-issue-model`；状态文件与任务板在本轮开始前已指向 `IMP-210`，工作树干净，可按唯一 execution unit 推进。 |
...
| 修改 | `docs/status/roadmap-state.yaml` / `docs/linear-parity/task-board.md` / `docs/planning/dev-logbook.md` / `docs/worktime.md` | 将 `IMP-210` 作为最新完成的 Implementation lane 最小增量写回，并补一条新的后继 `pending` Implementation 任务 `IMP-211` 供下一轮恢复。 |

### 323.2 本轮落地结果

- 已复核 helper 级标题仍通过 `returns the empty props contract when the helper workspace slug lookup misses` 明确保留 helper seam 语义。
- 在当前 `getIssue rejects` / `route getOrganizations rejects` 非对称 wording 组合下，第二条 `route getOrganizations rejects` 仍可再次收口为 `getOrganizations rejects`，同时继续维持 route 级 API 失败场景与 helper 级 lookup 标题之间的层级区分。
...
- `docs/status/roadmap-state.yaml`、`docs/linear-parity/task-board.md`、`docs/planning/dev-logbook.md` 与 `docs/worktime.md` 已写回：`IMP-210` 作为最新完成项，下一轮转入继续评估第一条 API 失败场景标题在当前再次双无 route wording 组合下是否需要恢复 route 级语义词。

### 323.3 经验沉淀

- 当第一条 legacy route API 失败标题已再次收口为无 route wording，而第二条仍保留 route wording 时，应单独复核第二条是否仍可继续收口，而不是默认保留非对称 wording；关键判断标准仍是 helper seam 标题是否保持独立 lookup 语义。
- 纯测试标题微调依然要在状态文件与任务板中立即补出后继 pending Implementation 任务，避免下一轮实现 cron 停在已完成项。
...
| 指标 | 值 |
|------|-----|
| 当前 lane / task | `Implementation / IMP-210 → IMP-211` |
| 当前分支 / HEAD（执行前） | `codex/unify-issue-model` / `e10536e3f7b687845f18225f1820518dd1933eb6` |
| feature commit | `f5bcd37d4b61e4304afcddfe2e9f147c569f7a1b` |
| 验证 | `pnpm test -- --run src/lib/routes.test.tsx`、`./node_modules/.bin/tsc --noEmit`、`git diff --check` |
...
**目标**：按 Implementation lane 推进 `docs/status/roadmap-state.yaml` 与 `docs/linear-parity/task-board.md` 指向的 `IMP-209` 最小增量，在检查 git/roadmap/task-board/logbook/worktime 后，只做一个最小 execution unit：基于当前 `route getIssue rejects` / `route getOrganizations rejects` 组合，复核第一条 API 失败场景标题是否仍可再次收口为 `getIssue rejects` 以继续维持与 helper 级 lookup 标题的层级区分；若可行，则仅改这一处标题并完成验证、review、提交、状态写回与 push 闭环。

### 300.1 执行步骤

| 步骤 | 操作 | 结果 |
|------|------|------|
| 恢复状态 | `git status --short` / `git branch --show-current` / `git rev-parse HEAD` / `git log --oneline -5` / 读取 `docs/status/roadmap-state.yaml` / `docs/linear-parity/task-board.md` / roadmap | 确认仓库位于 `codex/unify-issue-model`；状态文件与任务板指向 `IMP-209`，工作树干净，可按唯一 execution unit 推进。 |
| 实现 | `frontend/src/lib/routes.test.tsx` | 仅将 legacy `/issues/[id]` route 第一条 API 失败场景标题从 `route getIssue rejects` 再次收口为 `getIssue rejects`；保持第二条 `route getOrganizations rejects`、共享断言、fixture 设置与 helper/route 运行时逻辑不变。 |
| 验证 | `cd frontend && pnpm test -- --run src/lib/routes.test.tsx` | 通过：5 files / 57 tests |
| 验证 | `cd frontend && ./node_modules/.bin/tsc --noEmit` | 通过 |
| 验证 | `git diff --check` | 通过 |
| Review | `git diff -- frontend/src/lib/routes.test.tsx` | 复核本轮只改一处 legacy route 级 API 失败测试标题文案，不改共享断言结构、fixture 设置或 helper/route 运行时逻辑；review 结论为通过。 |
| 提交 | `git commit -m "[verified] test: restore first legacy route reject title again"` | 完成本轮 feature/work commit，提交后以 `git rev-parse HEAD` 记录真实 SHA。 |
| 修改 | `docs/status/roadmap-state.yaml` / `docs/linear-parity/task-board.md` / `docs/planning/dev-logbook.md` / `docs/worktime.md` | 将 `IMP-209` 作为最新完成的 Implementation lane 最小增量写回，并补一条新的后继 `pending` Implementation 任务 `IMP-210` 供下一轮恢复。 |

### 300.2 本轮落地结果

- 已复核 helper 级标题仍通过 `returns the empty props contract when the helper workspace slug lookup misses` 明确保留 helper seam 语义。
- 在当前 `route getIssue rejects` / `route getOrganizations rejects` 双 route wording 组合下，第一条 `route getIssue rejects` 仍可再次收口为 `getIssue rejects`，同时继续维持 route 级 API 失败场景与 helper 级 lookup 标题之间的层级区分。
- 因此本轮仅将第一条 route 级 API 失败场景测试标题再次收口为 `getIssue rejects`，同时保持第二条 `route getOrganizations rejects`、共享断言、fixture 设置以及 helper/route 运行时逻辑不变。
- `docs/status/roadmap-state.yaml`、`docs/linear-parity/task-board.md`、`docs/planning/dev-logbook.md` 与 `docs/worktime.md` 已写回：`IMP-209` 作为最新完成项，下一轮转入继续评估第二条 API 失败场景标题在当前再次非对称 wording 组合下是否仍可收口为无 route wording。

### 300.3 经验沉淀

- 当两条 legacy route API 失败标题都带 route wording 时，应单独复核第一条是否仍可收口为无 route wording，而不是默认继续保持双 route wording；关键判断标准仍是 helper seam 标题是否保持独立 lookup 语义。
- 纯测试标题微调依然要在状态文件与任务板中立即补出后继 pending Implementation 任务，避免下一轮实现 cron 停在已完成项。
...
| 指标 | 值 |
|------|-----|
| 当前 lane / task | `Implementation / IMP-209 → IMP-210` |
| 当前分支 / HEAD（执行前） | `codex/unify-issue-model` / `749a8df782b2a33a01f0fcb16e18741a2a14942e` |
| feature commit | `5aee662051353c2e1afc2de26b7540e7c71c0b9a` |
| 验证 | `pnpm test -- --run src/lib/routes.test.tsx`、`./node_modules/.bin/tsc --noEmit`、`git diff --check` |


## Session 322 — 2026-04-21：重新评估并恢复第二条 legacy route API 失败标题中的 route wording

**目标**：按 Implementation lane 推进 `docs/status/roadmap-state.yaml` 与 `docs/linear-parity/task-board.md` 指向的 `IMP-208` 最小增量，在检查 git/roadmap/task-board/logbook/worktime 后，只做一个最小 execution unit：基于当前 `route getIssue rejects` / `getOrganizations rejects` 组合，复核第二条 API 失败场景标题是否需要恢复为 `route getOrganizations rejects` 以继续维持与 helper 级 lookup 标题的层级区分；若需要，则仅改这一处标题并完成验证、review、提交、状态写回与 push 闭环。

### 322.1 执行步骤

| 步骤 | 操作 | 结果 |
|------|------|------|
| 恢复状态 | `git status --short` / `git branch --show-current` / `git rev-parse HEAD` / `git log --oneline -5` / 读取 `docs/status/roadmap-state.yaml` / `docs/linear-parity/task-board.md` / roadmap / `docs/planning/dev-logbook.md` / `docs/worktime.md` | 确认仓库位于 `codex/unify-issue-model`；状态文件与任务板在本轮开始前已指向 `IMP-208`，工作树干净，可按唯一 execution unit 推进。 |
| 实现 | `frontend/src/lib/routes.test.tsx` | 仅将 legacy `/issues/[id]` route 第二条 API 失败场景标题从 `getOrganizations rejects` 恢复为 `route getOrganizations rejects`；保持第一条 `route getIssue rejects`、共享断言、fixture 设置与 helper/route 运行时逻辑不变。 |
| 验证 | `cd frontend && pnpm test -- --run src/lib/routes.test.tsx` | 通过：5 files / 57 tests |
| 验证 | `cd frontend && ./node_modules/.bin/tsc --noEmit` | 通过 |
| 验证 | `git diff --check` | 通过 |
...
| 提交 | `git commit -m "[verified] test: restore second legacy route reject title again"` | 完成本轮 feature/work commit，提交后以 `git rev-parse HEAD` 记录真实 SHA。 |
| 修改 | `docs/status/roadmap-state.yaml` / `docs/linear-parity/task-board.md` / `docs/planning/dev-logbook.md` / `docs/worktime.md` | 将 `IMP-208` 作为最新完成的 Implementation lane 最小增量写回，并补一条新的后继 `pending` Implementation 任务 `IMP-209` 供下一轮恢复。 |

### 322.2 本轮落地结果

- 已复核 helper 级标题仍通过 `returns the empty props contract when the helper workspace slug lookup misses` 明确保留 helper seam 语义。
- 在当前 `route getIssue rejects` / `getOrganizations rejects` 非对称 wording 组合下，第二条 API 失败标题若继续维持无 route 级语义词，会破坏两条 route 级 API 失败场景与 helper 级 lookup 标题之间的最小对称层级区分。
- 因此本轮仅将第二条 route 级 API 失败场景测试标题恢复为 `route getOrganizations rejects`，同时保持第一条 `route getIssue rejects`、共享断言、fixture 设置以及 helper/route 运行时逻辑不变。
- `docs/status/roadmap-state.yaml`、`docs/linear-parity/task-board.md`、`docs/planning/dev-logbook.md` 与 `docs/worktime.md` 已写回：`IMP-208` 作为最新完成项，下一轮转入重新评估当前再次双 route wording 组合下第一条 API 失败场景标题是否仍可收口为无 route wording。

### 322.3 经验沉淀

- 当前非对称 wording 组合下，第二条 API 失败标题也需要单独复核；若第一条已恢复 route 级语义词，第二条继续维持无 route wording 可能会破坏 route 层测试标题的最小对称性。
...
| 指标 | 值 |
|------|-----|
| 当前 lane / task | `Implementation / IMP-208 → IMP-209` |
| 当前分支 / HEAD（执行前） | `codex/unify-issue-model` / 4898e256ec7f5f0a5f534fe9c3dac57395e7c334 |
| feature commit | `898dad0683330e8ac10ac51f5a7f6c54302fb99e` |
| 验证 | `pnpm test -- --run src/lib/routes.test.tsx`、`./node_modules/.bin/tsc --noEmit`、`git diff --check` |

---

## Session 321 — 2026-04-21：重新评估并再次收口第二条 legacy route API 失败标题中的 route wording

**目标**：按 Implementation lane 推进 `docs/status/roadmap-state.yaml` 与 `docs/linear-parity/task-board.md` 指向的 `IMP-206` 最小增量，在检查 git/roadmap/task-board/logbook/worktime 后，只做一个最小 execution unit：基于当前 `getIssue rejects` / `route getOrganizations rejects` 组合，复核第二条 API 失败场景标题是否仍可再次收口为 `getOrganizations rejects` 以继续维持与 helper 级 lookup 标题的层级区分；若可行，则仅改这一处标题并完成验证、review、提交、状态写回与 push 闭环。

### 321.1 执行步骤

| 步骤 | 操作 | 结果 |
|------|------|------|
| 恢复状态 | `git status --short` / `git branch --show-current` / `git rev-parse HEAD` / `git log --oneline -5` / 读取 `docs/status/roadmap-state.yaml` / `docs/linear-parity/task-board.md` / roadmap / `docs/planning/dev-logbook.md` / `docs/worktime.md` | 确认仓库位于 `codex/unify-issue-model`；状态文件与任务板在本轮开始前已指向 `IMP-206`，工作树干净，可按唯一 execution unit 推进。 |
| 实现 | `frontend/src/lib/routes.test.tsx` | 仅将 legacy `/issues/[id]` route 第二条 API 失败场景标题从 `route getOrganizations rejects` 再次收口为 `getOrganizations rejects`；保持第一条 `getIssue rejects`、共享断言、fixture 设置与 helper/route 运行时逻辑不变。 |
| 验证 | `cd frontend && pnpm test -- --run src/lib/routes.test.tsx` | 通过：5 files / 57 tests |
| 验证 | `cd frontend && ./node_modules/.bin/tsc --noEmit` | 通过 |
| 验证 | `git diff --check` | 通过 |
| Review | `git diff -- frontend/src/lib/routes.test.tsx` | 复核本轮只改一处 route 级 API 失败测试标题文案，不改共享断言结构、fixture 设置或 helper/route 运行时逻辑；独立 reviewer 判定无安全或逻辑问题，review 结论为通过。 |
| 提交 | `git commit -m "[verified] test: drop second legacy route reject title again"` | 完成本轮 feature/work commit，提交后以 `git rev-parse HEAD` 记录真实 SHA。 |
| 修改 | `docs/status/roadmap-state.yaml` / `docs/linear-parity/task-board.md` / `docs/planning/dev-logbook.md` / `docs/worktime.md` | 将 `IMP-206` 作为最新完成的 Implementation lane 最小增量写回，并补一条新的后继 `pending` Implementation 任务 `IMP-207` 供下一轮恢复。 |
...
- 因此本轮仅将第二条 route 级 API 失败场景测试标题再次收口为 `getOrganizations rejects`，同时保持第一条 `getIssue rejects`、共享断言、fixture 设置以及 helper/route 运行时逻辑不变。
- `docs/status/roadmap-state.yaml`、`docs/linear-parity/task-board.md`、`docs/planning/dev-logbook.md` 与 `docs/worktime.md` 已写回：`IMP-206` 作为最新完成项，下一轮转入重新评估当前再次双无 route wording 组合下第一条 API 失败场景标题是否需要恢复 route 级语义词。

### 321.3 经验沉淀

- 当前非对称 wording 组合下，第二条 API 失败标题也需要单独复核；不能因为第一条已收口为无 route wording，就默认第二条仍必须保留 route 级语义词。
- 纯测试标题微调同样要走完整闭环：验证、独立 review、feature commit、状态文件/任务板/logbook/worktime 回写缺一不可。
...
| 指标 | 值 |
|------|-----|
| 当前 lane / task | `Implementation / IMP-206 → IMP-207` |
| 当前分支 / HEAD（执行前） | `codex/unify-issue-model` / 005bb7a8a76087d93d2144921c26c8e74d277742 |
| feature commit | `ca5e96eea70d3130dd005a7aaf25e6806b2c5a78` |
| 验证 | `pnpm test -- --run src/lib/routes.test.tsx`、`./node_modules/.bin/tsc --noEmit`、`git diff --check` |

---

## Session 320 — 2026-04-21：重新评估并再次收口第一条 legacy route API 失败标题中的 route wording

**目标**：按 Implementation lane 推进 `docs/status/roadmap-state.yaml` 与 `docs/linear-parity/task-board.md` 指向的 `IMP-205` 最小增量，在检查 git/roadmap/task-board/logbook/worktime 后，只做一个最小 execution unit：基于当前 `route getIssue rejects` / `route getOrganizations rejects` 组合，复核第一条 API 失败场景标题是否仍可再次收口为 `getIssue rejects` 以继续维持与 helper 级 lookup 标题的层级区分；若可行，则仅改这一处标题并完成验证、review、提交、状态写回与 push 闭环。

### 320.1 执行步骤

| 步骤 | 操作 | 结果 |
|------|------|------|
| 恢复状态 | `git status --short` / `git branch --show-current` / `git rev-parse HEAD` / `git log --oneline -5` / 读取 `docs/status/roadmap-state.yaml` / `docs/linear-parity/task-board.md` / roadmap / `docs/planning/dev-logbook.md` / `docs/worktime.md` | 确认仓库位于 `codex/unify-issue-model`；状态文件与任务板在本轮开始前已指向 `IMP-205`，工作树干净，可按唯一 execution unit 推进。 |
| 实现 | `frontend/src/lib/routes.test.tsx` | 仅将 legacy `/issues/[id]` route 第一条 API 失败场景标题从 `route getIssue rejects` 再次收口为 `getIssue rejects`；保持第二条 `route getOrganizations rejects`、共享断言、fixture 设置与 helper/route 运行时逻辑不变。 |
| 验证 | `cd frontend && pnpm test -- --run src/lib/routes.test.tsx` | 通过：5 files / 57 tests |
| 验证 | `cd frontend && ./node_modules/.bin/tsc --noEmit` | 通过 |
| 验证 | `git diff --check` | 通过 |
| Review | `git diff -- frontend/src/lib/routes.test.tsx` | 复核本轮只改一处 route 级 API 失败测试标题文案，不改共享断言结构、fixture 设置或 helper/route 运行时逻辑；独立 reviewer 判定无安全或逻辑问题，review 结论为通过。 |
| 提交 | `git commit -m "[verified] test: drop first legacy route reject title again"` | 完成本轮 feature/work commit，提交后以 `git rev-parse HEAD` 记录真实 SHA。 |
| 修改 | `docs/status/roadmap-state.yaml` / `docs/linear-parity/task-board.md` / `docs/planning/dev-logbook.md` / `docs/worktime.md` | 将 `IMP-205` 作为最新完成的 Implementation lane 最小增量写回，并补一条新的后继 `pending` Implementation 任务 `IMP-206` 供下一轮恢复。 |
...
- 因此本轮仅将第一条 route 级 API 失败场景测试标题再次收口为 `getIssue rejects`，同时保持第二条 `route getOrganizations rejects`、共享断言、fixture 设置以及 helper/route 运行时逻辑不变。
- `docs/status/roadmap-state.yaml`、`docs/linear-parity/task-board.md`、`docs/planning/dev-logbook.md` 与 `docs/worktime.md` 已写回：`IMP-205` 作为最新完成项，下一轮转入重新评估当前再次非对称 wording 组合下第二条 API 失败场景标题是否仍可收口为无 route wording。

### 320.3 经验沉淀

- 即使双 route wording 组合已恢复，也需单独复核第一条 API 失败标题是否仍可再次收口；不能因为 helper seam wording 已清晰，就默认 route wording 必须全量保留。
- 纯测试标题微调同样要走完整闭环：验证、独立 review、feature commit、状态文件/任务板/logbook/worktime 回写缺一不可。
...
| 指标 | 值 |
|------|-----|
| 当前 lane / task | `Implementation / IMP-205 → IMP-206` |
| 当前分支 / HEAD（执行前） | `codex/unify-issue-model` / 090dfe370a56286fbb5a29ab481eea0eaeeb153c |
| feature commit | `368f8a896024bd2c8fbbab336c78852e33f6dd20` |
| 验证 | `pnpm test -- --run src/lib/routes.test.tsx`、`./node_modules/.bin/tsc --noEmit`、`git diff --check` |
