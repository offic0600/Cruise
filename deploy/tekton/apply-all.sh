...
echo "=== 3. 检查 Cruise 应用 secret ==="
SECRET_FILE="$PROJECT_ROOT/deploy/k3s/secret.yaml"
if [[ ! -f "$SECRET_FILE" ]]; then
  echo "未找到 $SECRET_FILE"
  echo "请先基于 deploy/k3s/secret.example.yaml 复制并填写真实密钥"
  exit 1
fi

echo "=== 4. 部署 Cruise 测试环境资源 ==="
kubectl apply -f "$PROJECT_ROOT/deploy/k3s/configmap.yaml"
kubectl apply -f "$SECRET_FILE"
kubectl apply -f "$PROJECT_ROOT/deploy/k3s/postgres-pvc.yaml"
kubectl apply -f "$PROJECT_ROOT/deploy/k3s/postgres.yaml"
kubectl apply -f "$PROJECT_ROOT/deploy/k3s/backend-pvc.yaml"
kubectl apply -f "$PROJECT_ROOT/deploy/k3s/backend.yaml"
kubectl apply -f "$PROJECT_ROOT/deploy/k3s/frontend.yaml"
kubectl apply -f "$PROJECT_ROOT/deploy/k3s/ingress.yaml"

echo "=== 5. 安装 Tekton Tasks ==="
kubectl apply -f "$PROJECT_ROOT/deploy/tekton/tasks/gradle-build.yaml"
kubectl apply -f "$PROJECT_ROOT/deploy/tekton/tasks/node-build.yaml"
kubectl apply -f "$PROJECT_ROOT/deploy/tekton/tasks/image-build.yaml"
kubectl apply -f "$PROJECT_ROOT/deploy/tekton/tasks/kubectl-set-image.yaml"

echo "=== 6. 安装 Cruise Pipelines ==="
