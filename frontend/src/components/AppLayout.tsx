...
import { publicPath, teamActivePath, teamSettingsPath, workspaceProjectsAllPath, workspaceRootPath, workspaceSectionPath, workspaceViewsPath } from '@/lib/routes';
...
const ISSUE_WORKSPACE_ROUTE = /\/team\/[^/]+\/(active|all|backlog|done)(\/|$)/;
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = useMemo<NavItem[]>(
    () => [
      ...
      { href: '/agent', label: t('nav.agent'), icon: 'M6 12a1.5 1.5 0 113 0 1.5 1.5 0 01-3 0zm4.5 0a1.5 1.5 0 113 0 1.5 1.5 0 01-3 0zm4.5 0a1.5 1.5 0 113 0 1.5 1.5 0 01-3 0z' },
    ],
    [t]
  );
  const isIssueWorkspaceRoute = ISSUE_WORKSPACE_ROUTE.test(pathname);
  ...
  const resolveNavHref = (item: NavItem) =>
    item.href === 'team-active'
      ...
      : item.href === '/team-members'
        ? (currentOrganizationSlug ? workspaceSectionPath(currentOrganizationSlug, 'team-members') : '#')
      : item.href === '/projects'
  ...
  const isNavActive = (item: NavItem, href: string) =>
    href !== '#'
      && (
        ...
        || (item.href === '/views/issues' && currentOrganizationSlug != null && pathname.startsWith(`/${currentOrganizationSlug}/views/`))
      );

  return (
    <div className="app-shell">
      <aside className={`glass-sidebar fixed inset-y-0 left-0 z-30 ${isIssueWorkspaceRoute ? 'w-[244px]' : 'w-72'} transition-transform xl:translate-x-0 ${mobileNavOpen ? 'translate-x-0' : '-translate-x-full'} xl:block`}>
        <div className="flex h-full flex-col">
          {isIssueWorkspaceRoute ? (
            <div className="border-b border-border-subtle px-4 py-4">
              <button
                type="button"
                onClick={() => setWorkspaceMenuOpen((current) => !current)}
                className="flex w-full items-center justify-between rounded-[10px] px-2 py-2 text-left transition hover:bg-white/70"
              >
                <div className="min-w-0">
                  <div className="truncate text-sm font-semibold text-[#23252a]">{currentOrganization?.name ?? t('common.workspace')}</div>
                  <div className="truncate text-xs text-[#8f95a3]">{currentTeam?.name ?? currentTeamKey ?? ''}</div>
                </div>
                <svg className="h-4 w-4 text-[#8f95a3]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="mt-3 flex items-center gap-2">
                <Button
                  variant="secondary"
                  size="icon"
                  className="h-8 w-8 rounded-[10px] border border-border-soft bg-white text-[#5b5b5d] shadow-none hover:bg-slate-50"
                ...
                <Button
                  ...
                  aria-label={t('workspaceMenu.newIssue')}
                >
                ...
                <Button
                  ...
                  aria-label={t('workspaceMenu.settings')}
                >
          ...
          <nav className={`flex-1 ${isIssueWorkspaceRoute ? 'space-y-3 px-3 py-4' : 'space-y-6 px-4 py-5'}`}>
          ...
          <div className={`border-t border-border-subtle ${isIssueWorkspaceRoute ? 'px-3 py-3' : 'px-4 py-4'}`}>
            <div className={isIssueWorkspaceRoute ? 'rounded-[10px] border border-border-soft bg-white p-3' : 'subtle-card p-4'}>
              <div className="text-sm font-medium text-ink-900">{user.username}</div>
              <div className="mt-1 text-xs text-ink-700">{user.role}</div>
              <div className={isIssueWorkspaceRoute ? 'mt-3' : 'mt-4'}>
                <LocaleSwitcher />
              </div>
            </div>
          </div>
      ...
      <div className={isIssueWorkspaceRoute ? 'xl:ml-[244px]' : 'xl:ml-72'}>
        <main className={isIssueWorkspaceRoute ? 'px-3 py-3 sm:px-4 lg:px-5' : 'px-5 py-6 sm:px-8'}>
