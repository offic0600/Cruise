...
import { getProjectInitials } from './project-composer-utils';
...
export function ProjectLeadSelectMenu({
  value,
  members,
  currentUser,
  query,
  ...
  currentUser?: ProjectLeadOption | null;
  query: string;
  onQueryChange: (value: string) => void;
  onSelect: (leadId: number | null) => void | Promise<void>;
  placeholder: string;
  shortcut: string;
  t: (key: string, vars?: Record<string, string | number>) => string;
}) {
  const normalizedQuery = query.trim().toLowerCase();
  const currentUserVisible = currentUser && (!normalizedQuery || currentUser.name.toLowerCase().includes(normalizedQuery));
    ...
    <>
      ...
      <div className="px-1 py-1.5">
        ...
        >
          ...
          <span className="truncate">{t('projects.composer.noLead')}</span>
          <span className="ml-auto flex items-center gap-2 text-sm text-ink-500">
            <span className="inline-flex w-4 justify-center">{value == null ? <Check className="h-4 w-4 text-ink-700" /> : null}</span>
            <span>0</span>
          </span>
        </button>
        {currentUserVisible ? (
          ...
          >
            ...
            <span className="ml-auto flex items-center gap-2 text-sm text-ink-500">
              <span className="inline-flex w-4 justify-center">{String(value ?? '') === String(currentUser.id) ? <Check className="h-4 w-4 text-ink-700" /> : null}</span>
              <span>1</span>
            </span>
          </button>
        ) : null}
        {filteredMembers.length ? (
          <div className="px-3 pb-1 pt-2 text-xs font-medium uppercase tracking-[0.08em] text-ink-400">
            {t('projects.composer.teamMembers')}
          </div>
        ) : null}
        {filteredMembers.map((member, index) => (
          ...
          >
            ...
            <span className="ml-auto flex items-center gap-2 text-sm text-ink-500">
              <span className="inline-flex w-4 justify-center">{String(value ?? '') === String(member.id) ? <Check className="h-4 w-4 text-ink-700" /> : null}</span>
              <span>{index + (currentUserVisible ? 2 : 1)}</span>
            </span>
          </button>
        ))}
        {!filteredMembers.length && !currentUserVisible ? (
          <div className="px-3 py-4 text-sm text-ink-400">{t('projects.composer.noLeadResults')}</div>
        ) : null}
      </div>
    </>
  );
