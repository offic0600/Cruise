  ...
  return (
    <div ref={containerRef} className="relative" data-testid="markdown-editor-root">
      ...
      {slashMenuMounted && slashState.open && filteredCommands.length
        ? createPortal(
            <div
              ...
              role="menu"
              className="fixed z-[85] min-w-56 w-[280px] overflow-hidden rounded-card border border-border-subtle bg-surface-elevated p-1.5 shadow-elevated"
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            index === slashState.selectedIndex ? 'bg-[color:var(--interactive-hover)] text-ink-900' : 'hover:bg-[color:var(--interactive-hover)] focus:bg-[color:var(--interactive-hover)]',
                          )}
                          ...
                          {command.shortcut ? (
                            <span className="ml-3 inline-flex shrink-0 items-center gap-1 text-xs text-ink-400">
                              {command.shortcut.display.map((token) => (
                                <span key={`${command.id}-${token}`} className="ds-keycap rounded px-1.5 py-0.5 text-[11px] leading-none">
                                  {token}
                                </span>
                              ))}
                            </span>
      ...
      {slashMenuMounted && toolbarState.open
        ? createPortal(
            <div
              ...
              className="ds-elevated-toolbar fixed z-[88] inline-flex max-w-full items-center gap-1 rounded-full px-1.5 py-1"
            ...
            >
              ...
              >
                <DropdownMenuTrigger asChild>
                  <button
                    ...
                    className="inline-flex h-9 items-center gap-1 rounded-[10px] bg-[color:var(--interactive-hover)] px-3 text-[15px] text-ink-700 outline-none transition hover:bg-[color:var(--interactive-pressed)]"
                  ...
                  <div data-testid="markdown-toolbar-text-style-menu">
                    {textStyleCommands.map((command) => (
                        ...
                        className={cn(
                          ...
                          command.isActive?.(editor) ? 'bg-[color:var(--interactive-hover)] text-ink-900' : ''
                        )}
                      >
              ...
              <div className="mx-0.5 h-6 w-px bg-border-soft" />
              ...
              {orderedToolbarCommands.slice(0, 7).map((command, index) => (
                <Fragment key={command.id}>
                  <button
                    ...
                    onClick={() => void runCommand(command)}
                    className={cn(
                      'inline-flex h-9 min-w-9 items-center justify-center rounded-[10px] border border-transparent px-2.5 text-ink-700 transition hover:bg-[color:var(--interactive-hover)]',
                      command.isActive?.(editor) ? 'bg-[color:var(--interactive-hover)] text-ink-900' : ''
                    )}
                  ...
                  {index === 3 ? <div className="mx-0.5 h-6 w-px bg-border-soft" /> : null}
                </Fragment>
              ))}
              ...
              >
                <DropdownMenuTrigger asChild>
                  <button
                    ...
                    className={cn(
                      'inline-flex h-9 items-center gap-1 rounded-[10px] border border-transparent px-2.5 text-ink-700 outline-none transition hover:bg-[color:var(--interactive-hover)]',
                      activeListCommand?.isActive?.(editor) ? 'bg-[color:var(--interactive-hover)] text-ink-900' : '',
                    )}
                  ...
                  <div data-testid="markdown-toolbar-list-menu">
                    {listCommands.map((command) => (
                      <DropdownMenuItem
                        ...
                        className={cn(
                          'flex items-center justify-between rounded-xl px-3 py-2.5 text-[15px]',
                          command.isActive?.(editor) ? 'bg-[color:var(--interactive-hover)] text-ink-900' : 'text-ink-700',
                        )}
                      >
                        ...
                        {command.shortcut ? (
                          <div className="flex items-center gap-1 text-[12px] text-ink-400">
                            {command.shortcut.display.map((token) => (
                              <span key={`${command.id}-${token}`} className="ds-keycap rounded-md px-1.5 py-0.5 leading-none">
                                {token}
                              </span>
                            ))}
                          </div>
      ...
      {slashMenuMounted && linkState.open
        ? createPortal(
            <div
              className="fixed z-[90] w-[320px] rounded-[20px] border border-border-subtle bg-surface-elevated p-3 shadow-elevated"
            ...
            >
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-end gap-2">
                  <button
                    type="button"
                    className="rounded-full px-3 py-1.5 text-sm text-ink-500 transition hover:bg-[color:var(--interactive-hover)] hover:text-ink-900"
                    onClick={removeLink}
                  ...
                  <button
                    type="button"
                    className="rounded-full border border-border-soft px-3 py-1.5 text-sm text-ink-900 transition hover:bg-[color:var(--interactive-hover)]"
                    onClick={applyLink}
