          ...
          <section className="space-y-3">
            ...
            {initiatives.map((initiative) => (
              <button
                ...
                className={`w-full rounded-panel border p-5 text-left transition ${selected?.id === initiative.id ? 'border-[color:var(--bg-inverse)] bg-[color:var(--bg-inverse)] text-[color:var(--fg-inverse)] shadow-nav' : 'border-border-subtle bg-surface-raised text-ink-900 shadow-card hover:bg-[color:var(--interactive-hover)]'}`}
              >
                <div className="text-sm font-semibold">{initiative.name}</div>
                <div className={`mt-2 text-xs ${selected?.id === initiative.id ? 'text-[color:color-mix(in_srgb,var(--fg-inverse)_72%,transparent)]' : 'text-ink-400'}`}>{initiative.slugId ?? `#${initiative.id}`}</div>
                <div className={`mt-3 text-sm ${selected?.id === initiative.id ? 'text-[color:color-mix(in_srgb,var(--fg-inverse)_72%,transparent)]' : 'text-ink-700'}`}>{initiative.description ?? t('initiatives.empty.noDescription')}</div>
              </button>
            ))}
          </section>
